...
function Paginate({ page, dispatch, jobs, globalPage, currentJobs, searching, description, location, full_time, dark }) {
  ...
  useEffect(() => {
    dispatch(getCurrentJobs((page - ((globalPage - 1) * 5) - 1) * 10, ((page - ((globalPage - 1) * 5) - 1) * 10) + 10));
  }, [page, dispatch, jobs, globalPage]);
  return (
    <React.Fragment>
      <div className="w-full flex justify-center">
        <button
          className={`${dark ? 'bg-blue-800 text-white' : 'bg-white text-gray-700'} ${page > 1 ? 'hover:bg-blue-500 hover:text-white' : 'bg-gray-100 text-gray-500 cursor-not-allowed'} font-semibold rounded-full h-10 w-10 shadow`}
          disabled={!(page > 1)}
          onClick={() => dispatch(setPage(page - 1))}
        >
          <i className="fas fa-arrow-left"></i>
        </button>
        <div className={`${dark ? 'bg-blue-800' : 'bg-white'} shadow rounded-full mx-2 flex items-center`}>
          {
        ...
        <button
          className={`${dark ? 'bg-blue-800 text-white' : 'bg-white text-gray-700'} ${currentJobs && currentJobs.length === 10 ? 'hover:bg-blue-500 hover:text-white' : 'bg-gray-100 text-gray-500 cursor-not-allowed'} font-semibold rounded-full h-10 w-10 shadow`}
