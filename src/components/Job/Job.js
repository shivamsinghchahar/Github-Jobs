  ...
  return (
    <div className={`${dark ? 'bg-blue-800' : 'bg-white'} shadow flex flex-wrap py-4 px-3 rounded shadow-sm mb-2 lg:mb-4`}>
      <header className="w-full flex flex-wrap justify-between items-baseline lg:mb-2">
        <h2 className={`${dark ? 'text-white' : 'text-gray-900'} truncate w-2/3 text-md font-bold`}>
      ...
      <footer className="w-full flex justify-between mt-2">
        <div className={`flex flex-wrap items-center w-3/5 flex-between text-xs ${dark ? 'text-gray-200' : 'text-gray-600'}`}>
          {
            job ?
              (<>
                <span>
                  {job.company.length > 14 ? job.company.substring(0, 11) + '...' : job.company}
                </span>
        ...
        <div className={`${dark ? 'text-gray-200' : 'text-gray-600'} text-xs`}>
          ...
          <span>
            {
              job
                ? job.location.length > 12 ? job.location.substring(0, 8) + '...' : job.location
